...
export const updateInventoryData = createAsyncThunk(
  "inventoryDataList/updateInventoryData",
  async (data) => {
    try {
      const { token, ...dataNew } = data;
      const response = await axios.post(BASE_URL + UPDATE_INVENTORY, dataNew, {
        headers: {
...
export const fetchInventoryListData = createAsyncThunk(
  "inventoryDataList/fetchInventoryListData.",
  async (data) => {
    try {
      const { token, ...dataNew } = data;
      const response = await axios.post(BASE_URL + LIST_INVENTORY, dataNew, {
        headers: {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
        },
