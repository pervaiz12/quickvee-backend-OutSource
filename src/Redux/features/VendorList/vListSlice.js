...
export const fetchVendorsListData = createAsyncThunk('vendors/fetchVendorsListData.', async (data) => {
    try {
        const response = await axios.post(BASE_URL + LIST_ALL_VENDORS, data, { headers: { "Content-Type": "multipart/form-data" } })
       
        if (response.data.status === "true") {
            const listdata = [response.data.vendor_name_list , response.data.vendor_payout_list,response.data.states ]
