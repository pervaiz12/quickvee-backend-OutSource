...
import { BASE_URL, DETAIL_CATEGORY_SALE_REPORT} from "../../../Constants/Config"
...
export const fetchdetailCategorySaleData = createAsyncThunk('detailCategorySale/fetchdetailCategorySaleData.', async (data) => {
    try {
        const response = await axios.post(BASE_URL + DETAIL_CATEGORY_SALE_REPORT, data, { headers: { "Content-Type": "multipart/form-data" } })
        if (response.data.status === true) {
          ...
          } else if (response.data.status === false && response.data.message === "No data found") {
