...
const OrderSummaryDetails = () => {
  ...
  const order_id = location.pathname.replace('/store-reporting/order-summary/', '');
  ...
  const AllOrderSummaryDataState = useSelector((state) => state.orderSummeryList);
  useEffect(() => {
    if (!AllOrderSummaryDataState.loading && AllOrderSummaryDataState.OrderData) {
      // console.log(AllOrderSummaryDataState.OrderData)
      setOrderSummaryData(AllOrderSummaryDataState.OrderData);
      setMerchantDetailsData(AllOrderSummaryDataState.OrderData.merchant_details);
    }
  }, [
  ...
  useEffect(() => {
    if (OrderSummaryData && OrderSummaryData.order_detail && OrderSummaryData.order_detail.order_method) {
