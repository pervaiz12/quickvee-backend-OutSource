...
const OrderRefundReportList = (props) => {
    ...
    const calculateGrandTotal = (allOrderData, key) => {
        ...
        return allOrderData.reduce((total, order) => total + parseFloat(order[key]), 0);
    };
    

      const isAllOrderDataValid = Array.isArray(allOrderData) && allOrderData.length > 0;
                ...
                {isAllOrderDataValid ? (
                    <>
                        {allOrderData.map((CheckData, index) => {
                            const formattedDate = new Date(CheckData.created_at).toLocaleDateString('en-US', {
                                day: 'numeric',
                                month: 'short',
                                year: 'numeric',
                            });
                            ...
                            return (
                                ...
                                <div key={index} className="q-category-bottom-categories-listing">
                                    <div className="q-category-bottom-categories-single-category">
                                        ...
                                        <p className="report-title">${formatAmount(CheckData.debit_amt)}</p>
                                        <p className="report-title">${formatAmount(CheckData.cash_amt)}</p>
                                        <p className="report-title">${formatAmount(CheckData.loyalty_point_amt)}</p>
                                        <p className="report-title">${formatAmount(CheckData.store_credit_amt)}</p>
                                        <p className="report-title">${formatAmount(CheckData.nca_amt)}</p>
                                        <p className="report-title">${formatAmount(CheckData.tip_amt)}</p>
                                        <p className="report-title">${formatAmount(CheckData.amount)}</p>
                        ...
                        <div className="q-category-bottom-categories-listing">
                            <div className="q-category-bottom-categories-single-category">
                                ...
                                <p className="report-title">${parseFloat(calculateGrandTotal(allOrderData, 'amount')).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
