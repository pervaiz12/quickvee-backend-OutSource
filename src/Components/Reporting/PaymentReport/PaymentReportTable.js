...
const PaymentReportTable = (props) => {
  ...
  const totals = Object.values(props.paymentData).reduce(
    (acc, payment) => {
      acc.collected += payment.collected;
      acc.transactions += payment.transactions;
      acc.refunds += payment.refunds;
      acc.collected_transaction += payment.collected_transaction;
      acc.refund_transaction += payment.refund_transaction;
      return acc;
    },
    {
      collected: 0,
          ...
          <TableContainer>
            {props.paymentData.length !== 0 ? (
              <StyledTable sx={{ minWidth: 500 }} aria-label="customized table">
                <TableHead>
                  {RowHeader?.map((res, index) => {
                    return (
                      <StyledTableCell>
                        <p className="whitespace-nowrap">{res}</p>
                      </StyledTableCell>
                    );
                  })}
                  {/* <StyledTableCell>
                ...
                <TableBody>
                  ...
                  <StyledTableRow key="total">
                    ...
                    <StyledTableCell>
                      <p className="whitespace-nowrap">{totals.collected}</p>
                    </StyledTableCell>
                    <StyledTableCell>
                      <p className="whitespace-nowrap">{totals.transactions}</p>
                    </StyledTableCell>
                    <StyledTableCell>
                      <p className="whitespace-nowrap">{totals.refunds}</p>
                    </StyledTableCell>
                    <StyledTableCell>
                      <p className="whitespace-nowrap">
                        {totals.collected_transaction}
                      </p>
                    </StyledTableCell>
                    <StyledTableCell>
                      <p className="whitespace-nowrap">
                        {totals.refund_transaction}
                      </p>
                    </StyledTableCell>
                  </StyledTableRow>

                  {/* Map through paymentData */}
                  {Object.keys(props.paymentData).map(
                    (paymentMethod, index) => {
                      const {
                        collected = 0,
                        transactions = 0,
                      ...
                      return (
                        <StyledTableRow key={index}>
                          ...
                          <StyledTableCell>
                            <p className="whitespace-nowrap">{collected}</p>
                          </StyledTableCell>
                          <StyledTableCell>
                            <p className="whitespace-nowrap">{transactions}</p>
                          </StyledTableCell>
                          <StyledTableCell>
                            <p className="whitespace-nowrap">{refunds}</p>
                          </StyledTableCell>
                          <StyledTableCell>
                            <p className="whitespace-nowrap">
              ...
              props.paymentData.length == 0 && <NoDataFound />
            )}
          </TableContainer>
          {/* )} */}
        </Grid>
