...
export default function LoginLogic() {
  const navigate = useNavigate();
    ...
    const [formData,setFormData]=React.useState({username:'',password:'',otp:''})
    const [errors,setErrors]=React.useState({usernameError:'',passwordError:'',otpError:''})
    const{UsernameValidate,validatePassword,validateForm,validateOTP}=Validate({formData,setErrors,errors})
    ...
    const emailValidate = async (userdata) => {
        ...
        try {
          const response = await axios.post(BASE_URL+LOGIN_AUTHENICATE_API, data, {
