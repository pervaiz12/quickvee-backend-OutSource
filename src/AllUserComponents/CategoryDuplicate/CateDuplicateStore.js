...
import { BASE_URL , LIST_ALL_CATEGORIES_MECHANT_ID , CATEGORY_INVENTORY_DUPLICATE } from "../../Constants/Config";
...
const CateDuplicateStore = () => {
  ...
  const [storeFromDropdownVisible, setStoreFromDropdownVisible] = useState(false);
          ...
          if (option.merchant_id !== "-- Select Store --") {
            ...
            try {
              ...
              if (response.data.status === "Success") {
                const newCategoryOptions = response.data.result.map((category) => ({
              ...
              }else if(response.data.status === "Failed" && response.data.msg === "No Categorys found"){
                setCategoryOptions([{ value: "No categories found", label: "No categories found" }]);
                setSelectedCategories([]);
              }
            } catch (error) {
  ...
  const dupplicateCategoryInventory = async (e) =>{
    ...
    if (selectedStorefrom === "-- Select Store --") {
      ...
      const data = {
        ...
        try {
          const response = await axios.post(BASE_URL + CATEGORY_INVENTORY_DUPLICATE, data, { headers: { "Content-Type": "multipart/form-data" } })
  ...
  return (
    <>
              ...
              <div className="custom-dropdown"   >
                ...
                {storeFromDropdownVisible && (
                  <div className="dropdown-content" style={myStyles}>
                    <div onClick={() => handleOptionClick({ label: "-- Select Store --", merchant_id: null }, "storefrom")}>
                    -- Select Store --
                    </div>
                    {MerchantList && MerchantList.map((merchant) => (
                        <div key={merchant.id} onClick={() => handleOptionClick({ label: merchant.name, merchant_id: merchant.merchant_id }, "storefrom")}>
                          {merchant.name}
                        </div>
                      ))}
                  </div>
      ...
      />
              ...
              <div className="">
                ...
                <div className="custom-dropdown">
                  ...
                  {storeToDropdownVisible && (
                    <div className="dropdown-content" style={myStyles}>
                      <div onClick={() => handleOptionClick({ label: "-- Select Store --", merchant_id: null }, "storeto")}>
                      -- Select Store --
                      </div>
                      {MerchantList && MerchantList.map((merchant) => (
                          <div key={merchant.id} onClick={() => handleOptionClick({ label: merchant.name, merchant_id: merchant.merchant_id }, "storeto")}>
                          {merchant.name}
                        </div>
                      ))}
                    </div>
          ...
          >
            <button className="quic-btn quic-btn-save" onClick={dupplicateCategoryInventory}>
              Duplicate Inventory
            </button>
