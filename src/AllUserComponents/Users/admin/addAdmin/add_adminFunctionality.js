...
export default function Add_adminFunctionality() {
  const navigate = useNavigate();
    ...
    const handleChange = async (e) => {
      ...
      if (name === 'email') {
          errors[name] = value === '' ? `Please fill in the ${name} field` :
              !emailRegex.test(value) ? `Please give valid ${name} field` :'' ;
  ...
  function Currentvalidate(errors)
  {
    ...
    if((errors.owner_name=="") && (errors.email=="") &&(errors.password=="")&& (errors.phone==""))
    {
      return true
...
const handleSubmit=async(e)=>
{
    ...
    return {handleChange,addAdminData,handleSubmit,handleBlur,handleKeyPress}
